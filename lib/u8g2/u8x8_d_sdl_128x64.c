...
static void u8g_sdl_set_pixel(int x, int y, int idx) {
    ...
    if(y >= u8g_sdl_height) return;
    if(y < 0) return;
    if(x >= u8g_sdl_width) return;
    if(x < 0) return;
    ...
    for(i = 0; i < u8g_sdl_multiple; i++)
        for(j = 0; j < u8g_sdl_multiple; j++) {
            ...
            assert(
                offset < (Uint32)(u8g_sdl_width * u8g_sdl_multiple * u8g_sdl_height *
                                  u8g_sdl_multiple * u8g_sdl_screen->format->BytesPerPixel));
...
static void u8g_sdl_init(int width, int height) {
    ...
    u8g_sdl_color[0] = SDL_MapRGB(u8g_sdl_screen->format, 242, 140, 40);
    ...
    u8g_sdl_color[3] = SDL_MapRGB(u8g_sdl_screen->format, 0, 0, 0);
    u8g_sdl_color[4] = SDL_MapRGB(u8g_sdl_screen->format, 242, 140, 40);
